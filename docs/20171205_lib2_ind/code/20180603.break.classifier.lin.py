...
def plot_predictions(X_test, y_test, y_pred, train_str, test_str, plotfile):
  ...
  plt.plot(fpr, tpr, color='darkorange',
           lw=lw, label='(area = %0.2f)'.format(roc_auc))
  plt.plot([0, 1], [0, 1], color='navy', lw=lw, linestyle='--')
  plt.xlim([0.0, 1.0])
  plt.ylim([0.0, 1.05])
  plt.xlabel('False Positive Rate')
  plt.ylabel('True Positive Rate')
  main_title_str = 'ROC for Linear Classifier ("brokenness")\nAUC: {0.2f}'
  plt.title(main_title_str.format(roc_auc))
...
def apply_model(model, X_test):
  ...
  preds = [x['class_ids'][0] for x in model.predict(test_pred_input_func)]
  return preds
...
def plot_weights(model, feat_cols, plotfile):
  ...
  ax = sns.heatmap(hmframe) #, vmin=-1.1, vmax=1.1)
...
def train_eval_visualize(X_all, y_all, grouplabels, feat_cols):
  ...
  logging.info('Plotting idx/trans weights...'.format(**vars()))
...
if __name__ == '__main__':
  ...
  for span in spans:
    ...
    y_all = (usable_data[span].reset_index(drop=True) < -0.9)
