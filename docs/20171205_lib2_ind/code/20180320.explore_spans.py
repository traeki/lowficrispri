...
D['early'] = D_from_taggers(['glob', 'early'])
D['mid'] = D_from_taggers(['glob', 'mid'])
D['late'] = D_from_taggers(['glob', 'late'])
A = cleaned
...
for shared in ['early', 'mid', 'late']:
  ...
  residual[shared] = shared_projection - glob_projection

for name, residue in residual.iteritems():
  ...
  plt.suptitle(
      'global vs. residual of {name} span'.format(**vars()),
      fontsize=16)
  g.set_axis_labels('global', name + ' residual')
...
for xname, yname in [('early', 'mid'), ('early', 'late'), ('mid', 'late')]:
  ...
  xrep = residual[xname][:, xrelevant][:, 0]
  yrep = residual[yname][:, yrelevant][:, 0]
  fig = plt.figure(figsize=(6,6))
  g = sns.jointplot(xrep, yrep, s=2, linewidth=0.5, alpha=0.5)
  plt.suptitle(
      '{xname} residual vs. {yname} residual'.format(**vars()),
      fontsize=16)
  g.set_axis_labels(xname + ' residual', yname + ' residual')
