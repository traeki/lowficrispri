...
fcol_idx = tf.feature_column.categorical_column_with_vocabulary_list(
    'mismatch_idx', oneoff_features.mismatch_idx.unique())
fcol_trans = tf.feature_column.categorical_column_with_vocabulary_list(
    'mismatch_trans', oneoff_features.mismatch_trans.unique())
feat_cols = [fcol_idx, fcol_trans]

logging.info('Creating input functions'.format(**vars()))
# TODO(jsh): Change test_train_split call to correctly handle variant groups
chosen_span = '03'
X_all = oneoff_scored[featnames].reset_index(drop=True)
y_all = oneoff_scored[chosen_span].reset_index(drop=True)
_xxyy = train_test_split(X_all, y_all, test_size=0.3, random_state=42)
X_train, X_test, y_train, y_test = _xxyy
