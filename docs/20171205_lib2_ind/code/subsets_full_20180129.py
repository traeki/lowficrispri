...
samples over the complete run (T0 -> T3)?
'''
...
replicate_pairs = [('t0d3_a3d3', 't0d3_b3d3'),
                   ('t0d3_a3d3', 't0d3_c3d3'),
                   ('t0d3_b3d3', 't0d3_c3d3')]
labels = dict()
labels['t0d3_a3d3'] = 'No Drug - A'
labels['t0d3_b3d3'] = '30 ng/mL - B'
labels['t0d3_c3d3'] = '30 ng/mL - C'
...
for i, key in enumerate(subsets):
  for j, (rep_x, rep_y) in enumerate(replicate_pairs):
    ...
    sns.regplot(rep_x, rep_y, data=subset, fit_reg=False, ax=ax,
                label=key,
