...
sober_rel = np.asarray([relation(dose_none)]).T
image_sober, kernel_sober = im_ker_partition(sober_rel)

subspace_filtered = image_both(cleaned)
sober_piece = image_sober(cleaned)
dose_orthog = image_dose(kernel_glob(cleaned))
subspace_filtered = pd.DataFrame(subspace_filtered,
                                 index=cleaned.index,
                                 columns=cleaned.columns)
U_sober, s_sober, Vt_sober = np.linalg.svd(sober_piece, full_matrices=False)
...
sober_piece = pd.DataFrame(sober_piece,
                          index=cleaned.index,
                          columns=cleaned.columns)

best_gamma = sober_piece.mean(axis=1) * 54/30
best_gamma = best_gamma.reset_index()
