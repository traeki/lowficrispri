...
# Y is the full matrix
Y = scaler.fit_transform(cleaned.T).T
...
scaler_subset = preprocessing.StandardScaler(with_mean=True, with_std=False)
# X is the pre-filtered matrix
...
#### <Mask Code>
sample_ids = cleaned.columns.get_level_values(0)
dim = len(sample_ids)
...
def both_match(tags, item):
  return lambda i, j: (tags[i] == item) and (tags[j] == item)
...
samedose = np.fromfunction(np.vectorize(agreement(doses)),
                           (dim, dim), dtype=int)
no_drug = np.fromfunction(np.vectorize(both_match(doses, 0)),
                          (dim, dim), dtype=int)
#### </Mask Code>
...
S_m = np.diag(s_m)
S_m_inv = np.where(np.isclose(S_m, 0), 0, 1/S_m)
U_m = Y.dot(V_m.dot(S_m_inv))
