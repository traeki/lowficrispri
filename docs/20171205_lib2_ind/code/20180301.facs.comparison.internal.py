...
final = final.stack(level=[0,1,2]).unstack(level=1).unstack().unstack()

# Graph comparison
approaches = ['pool', 'facs']
spans = ['early', 'late']
cases = list(itertools.product(spans, approaches))
rows = len(cases)
doses = ['none', 'low', 'high']
dosepairs = list(itertools.combinations(doses, 2))
cols = len(dosepairs)
gsize = 4
fig, axes = plt.subplots(rows, cols, figsize=(gsize*cols,gsize*rows))
(start, stop) = (-1.5, .1)
for j, (xdose, ydose) in enumerate(dosepairs):
  for i, (span, approach) in enumerate(cases):
    ax = axes[i, j]
    ax.plot((start, stop), (start, stop), 'r--', linewidth=0.5)
    final[(span, approach)].plot(xdose, ydose, kind='scatter', ax=ax)
    ax.set_xlim(start, stop)
    ax.set_ylim(start, stop)
    ax.set(xlabel=xdose)
    ax.set(ylabel=ydose)
    ax.set(title='{span}: {approach}'.format(**vars()))
fig.tight_layout()
# fig.suptitle(
#     'gamma comparisons by timespan/approach'.format(**vars()), fontsize=16)
