...
def train_eval_visualize(X_train, y_train, trainkey,
  ...
  for key in keys:
    genegrouper = X_y_test.groupby(key)
    groupedfile = partnerfile('.'.join([threadlabel, key, 'mean', 'tsv']))
...
if __name__ == '__main__':
  ...
  span = '03'
  for trainkey, testkey in itertools.permutations(guidesets, 2):
    ...
    train_mask.index = usable_data.index
    test_mask = usable_data.reset_index().variant.apply(test_check)
