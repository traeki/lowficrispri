...
def plot_presence(frame, plotfile):
  ...
  tally.set_index(['trans', 'index'], inplace=True)
  tally = tally.unstack()
  plt.figure(figsize=(10,6))
  ax = sns.heatmap(tally, vmin=0, vmax=100)
  main_title_str = 'post-filter counts for [Base Index X Base Transition]'
...
if __name__ == '__main__':
  ...
  for span in spans:
    ...
    plotfile = partnerfile('png')
    plot_presence(usable_data, plotfile)
