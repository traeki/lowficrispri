...
def chooser(s, e):
  return (int(s[1]) == 0 and int(e[1]) == 3)
subset = [(s, e) for (s, e) in pairs if chooser(s, e)]
grid = sns.PairGrid(subspace_filtered, vars=subset)

def scatterwrapper(x, y, color, **kwargs):
  plt.scatter(x, y, **kwargs)
grid.map(scatterwrapper,
         s=2, linewidth=0.5, alpha=0.5, c=cc.m_inferno_r(colors.PC1))
