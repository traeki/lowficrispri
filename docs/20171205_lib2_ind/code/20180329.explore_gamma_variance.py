...
plotdf = backdf.reset_index().groupby('variant').agg([np.std, np.median])
plotdf.columns = ['std2', 'med2', 'std3', 'med3']
...
for x, y in xypairs:
  flatoutfile = partnerfile(x + y + '.png')
  if x.startswith('med'):
    xlim=(-2, 0.3)
  else:
    xlim=(-.01, .25)
  if y.startswith('med'):
    ...
    ylim=(-.01, .25)
  plotdf.plot.scatter(x, y, s=2, alpha=0.2, xlim=xlim, ylim=ylim)
  plt.savefig(flatoutfile)
  plt.clf()
