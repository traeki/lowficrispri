...
def compute_relative_gammas(rawdata, oneoffs, solo_gammas):
  """Compare gammas between "parent" and "child" strains.

  Args:
    oneoffs: two-column list of mismatch pairs
    solo_gammas: the individual gammas to be compared
  Returns:
...
def relative_gammas_from_raw_data(rawfile, oneoffs, oddatafile):
  ...
  return compute_relative_gammas(rawdata, oneoffs, solo_gammas)
...
def plot_subsets(gammas, firstbase):
  for base in firstbase.unique():
    plotfile = partnerfile(base + '.png')
    plt.figure(figsize=(6,6))
    subset = gammas.loc[firstbase == base]
    g = sns.distplot(subset)
    g.set(ylim=(-.1, 5), xlim=(-1.2, 0.2))
    plt.xlabel('gamma')
    plt.ylabel('count')
    main_title_str = 'gammas for leading base {base}'.format(**vars())
    plt.title(main_title_str)
    plt.savefig(plotfile)
    plt.clf()
...
if __name__ == '__main__':
  ...
  child_gammas = pd.merge(oneoffs, flatspans,
                          ...
                          how='left')
  child_gammas.set_index(['variant', 'original'], inplace=True)
  firstbase = parent_gammas.apply(lambda x: x.name[1][0], axis=1)
  span = '03'
  plot_subsets(parent_gammas[span], firstbase)
